package com.example.lesson_05_0916_thymeleafcrud.controller;
...
import com.example.lesson_05_0916_thymeleafcrud.repository.CustomerRepo;
...
//    }
    ...
    public String search(HttpServletRequest request,Model model){
        ...
        if (name == ""){
            ...
            model.addAttribute("customers",repo.findByName(name));
            return "list";
        }
    }
    @GetMapping("/create")
    ...
    @PostMapping("/post")
    public String postInfor(@ModelAttribute("customer") Customer customer, BindingResult result, Model model){
        if (!result.hasErrors()) {
            if(customer.getId()>0){
